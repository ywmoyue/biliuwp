    ...
    public sealed partial class MusicCollectPage : Page
    {
        ...
        private async void LoadSongs(string collectId)
        {
            ...
            try
            {
                ...
                url = string.Format(url, collectId, ApiHelper.access_key, ApiUtils.AndroidKey.Appkey, ApiHelper.GetUserId(), ApiHelper.GetTimeSpan);
                url += "&sign=" + ApiHelper.GetSign(url);
                var results = await WebClientClass.GetResults(new Uri(url));
        ...
        private async void DeleteCollect()
        {
            try
            {
                ...
                string content = string.Format("access_key={0}&appkey={1}&build=5250000&collectionId={2}&mid={3}&mobi_app=android&platform=android&songIds={4}&ts={5}", ApiHelper.access_key, ApiUtils.AndroidKey.Appkey, cid, ApiHelper.GetUserId(),Uri.EscapeDataString(songs), ApiHelper.GetTimeSpan);
