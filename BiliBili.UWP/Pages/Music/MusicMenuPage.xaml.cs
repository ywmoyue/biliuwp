    ...
    public sealed partial class MusicMenuPage : Page
    {
        ...
        private async void GetMenuInfo()
        {
            try
            {
                ...
                string url =string.Format( "https://api.bilibili.com/audio/music-service-c/menus/{3}?appkey={0}&build=5250000&mid={1}&mobi_app=android&platform=android&ts={2}",ApiUtils.AndroidKey.Appkey,ApiHelper.GetUserId(),ApiHelper.GetTimeSpan, _menuId);
        ...
        private async void doCollect(string mode)
        {
            ...
            try
            {
                ...
                string url = string.Format("https://api.bilibili.com/audio/music-service-c/menucollect/{0}?access_key={1}&appkey={2}&build=5250000&menuId={3}&mid={4}&platform=android&ts={5}",
                    mode,ApiHelper.access_key,ApiUtils.AndroidKey.Appkey, _menuId, ApiHelper.GetUserId(), ApiHelper.GetTimeSpan);
                url += "&sign=" + ApiHelper.GetSign(url);
                var re = await WebClientClass.GetResults(new Uri(url));
